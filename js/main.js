...
function loginBtnClicked() {
    ...
    axios.post(`${baseUrl}/login`, params)
        .then((response) => {
            const loginModal = FlowbiteInstances.getInstance('Modal', 'login-modal'); // Instantiate a new Modal object
...
function singupBtnClicked() {
    ...
    axios.post(`${baseUrl}/register`, formData)
        .then((response) => {
            const singupModal = FlowbiteInstances.getInstance('Modal', 'singup-modal'); // Instantiate a new Modal object
            console.log(response.status);
            localStorage.setItem("token", response.data.token);
            localStorage.setItem("user", JSON.stringify(response.data.user));
...
/*  add post btn  */
document.getElementById("add_post_btn").addEventListener("click", addPostClicked);
...
function addPostBtnClicked (){
    document.getElementById("add_post_btn").innerHTML = "Add"
    document.getElementById("post-modal-title").innerHTML = "Add post";
...
function addPostClicked() {
    const addPostModal = FlowbiteInstances.getInstance('Modal', 'add-post-modal'); // Instantiate a new Modal object
    ...
    if (isCreate) {
        url = `${baseUrl}/posts`;
        message = "You are add post successfully";
    }
    else {
        formData.append("_method", "PUT");
        url = `${baseUrl}/posts/${postId}`;
        message = "You are edit post successfully";
    }
...
console.log("addPostClicked function ended");
/* infinte scroling  */
let currentPage = 1;
let lastPage = 1;
...
function showAlert(content, type) {
    let alertcontent = `
        <div class="bg-${type}-100 border border-${type}-400 text-${type}-700 px-4 py-3 fixed bottom-10 right-0 -translate-y-[90%] z-[1100]  inline-flex w-fit items-center justify-center flex-wrap rounded-lg bg-success-100 sm:px-2 text-base " role="alert">
            ${content}
        </div>
    `;
...
function editPostBtnClickd(postObj) {
    ...
    document.getElementById("add_post_btn").innerHTML = "Update"
    document.getElementById("post-modal-title").innerHTML = "Edit post";
    document.getElementById("post_id").value = post.id;
    document.getElementById("post_title").value = post.title;
    document.getElementById("post_description").value = post.body;

    let addPostModal = FlowbiteInstances.getInstance('Modal', 'add-post-modal'); // Instantiate a new Modal object
    addPostModal.toggle();
}
function deletPostBtnClickd(postObj) {
    ...
    console.log(document.getElementById("delete-post-modal"));
    const deleteModal =  new Modal(document.getElementById("delete-post-modal"),{});
    deleteModal.toggle();
}

function confermDeletePostBtnClicked() {
    let deletePostModal = new Modal(document.getElementById("delete-post-modal"),{}); // Instantiate a new Modal object
...
function profileClicked() {
    window.location.href = `/profile.html?id=${getCurentUser().id}`;
}